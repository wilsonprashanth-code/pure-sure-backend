...
df_red_export = pd.DataFrame(base_recipes_rows)
out_red_path = 'Base_Recipes_49_Red_Sheets.xlsx'
df_red_export.to_excel(out_red_path, index=False)
print(f"Successfully generated {out_red_path} with {len(df_red_export)} rows across {len(red_sheets)} red sheets.")
